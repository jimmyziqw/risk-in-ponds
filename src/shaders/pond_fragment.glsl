...
vec3 getSurfaceRayColor(vec3 origin, vec3 light, vec3 ray, vec3 waterColor, vec3 squareCenter, sampler2D petalMap) {
    ...
    // float pondSize = 5.0;
    float l = pondSize * 0.5;
    float w = pondSize * 0.5;
...
vec4 addLight(sampler2D map, vec2 coord, vec3 position, vec3 eye) {
    float delta = 0.001;
    ...
    if(position.x > pondSize * 0.5 - delta || position.z < -pondSize * 0.5 + delta) { //bypass lightupdate
...
void main() {    
    ...
    vec4 res = addLight(map, vUv, vPosition, cameraPosition);
